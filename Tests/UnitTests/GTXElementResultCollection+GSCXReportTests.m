...
@interface GTXElementResultCollectionGSCXReportTests : XCTestCase
@end

@implementation GTXElementResultCollectionGSCXReportTests

- (void)testGTXElementResultCollectionCanProvideHTMLDescriptions {
  ...
  for (GTXElementResultCollection *elementResult in @[ elementResult1, elementResult2 ]) {
    NSString *htmlDescription = [elementResult htmlDescription];
    for (GTXCheckResult *checkResult in elementResult.checkResults) {
      XCTAssertTrue([htmlDescription containsString:checkResult.checkName]);
      assertionCount += 1;
    }
  }
